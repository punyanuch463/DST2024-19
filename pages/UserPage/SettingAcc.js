...
const SettingAccount = () => {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const formattedDob = new Date(formData.birthday)
        .toISOString()
        .split("T")[0];
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (
        ...
        updatedFormData.address = `${updatedFormData.houseNumber || ""} ${updatedFormData.street || ""} ${updatedFormData.district|| ""} ${updatedFormData.city || ""} ${updatedFormData.province || ""} ${updatedFormData.zip_code || ""}`.trim();
      }
    ...
    if (id === "birthday") {
      ...
      setFormData({ ...formData, birthday: value, age: age });
    }
  };
...
export default withAuth(SettingAccount);
